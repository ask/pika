...
from pika import event
from pika import exception
...
class ChannelHandler(object):
    ...
    def _ensure(self):
        if self.channel_close:
            raise exceptions.ChannelClosed(self.channel_close)
        return self
    ...
    def wait_for_reply(self, acceptable_replies):
        ...
        if self.reply_map is not None:
            raise exceptions.RecursiveOperationDetected([p.NAME
                    for p in acceptable_replies])
    ...
    def _handle_async(self, method_frame, header_frame, body):
        ...
        if self.reply_map is not None and method_cls in self.reply_map:
            if header_frame is not None:
                method._set_content(header_frame.properties, body)
            handler = self.reply_map[method_cls]
            self.reply_map = None
            handler(method)
        elif method_cls in self.async_map:
            self.async_map[method_cls](method_frame, header_frame, body)
        else:
            self.connection.close(spec.NOT_IMPLEMENTED,
                    "Pika: method not implemented: %s" % (method_cls.NAME, ))
...
class Channel(spec.DriverMixin):
    ...
    def __init__(self, handler):
        ...
        handler.async_map.update({
            spec.Channel.Close: handler._async_channel_close,
            spec.Channel.Flow: handler._async_channel_flow,
            spec.Basic.Deliver: handler._async_basic_deliver,
            spec.Basic.Return: handler._async_basic_return,
        })

        self.handler._rpc(spec.Channel.Open(), [spec.Channel.OpenOk])

    def addStateChangeHandler(self, handler, key=None):
        # TODO state_change_handlers.append(handler, key) ?
    ...
    def _async_basic_deliver(self, method_frame, header_frame, body):
        ...
        If a particular consumer is still active when another delivery
        appears for it, queue the deliveries up until it finally exits.
    ...
    def basic_publish(self, exchange, routing_key, body, properties=None,
            mandatory=False, immediate=False, block_on_flow_control=False):
        if self.handler.content_transmission_forbidden():
            if block_on_flow_control:
                ...
                raise exceptions.ContentTransmissionForbidden(self)
        if properties is None:
            properties = spec.BasicProperties()
        method = spec.Basic.Publish(exchange=exchange,
    ...
    def basic_consume(self, consumer, queue='', no_ack=False, exclusive=False,
        ...
        if tag in self.callbacks:
            raise exceptions.DuplicateConsumerTag(tag)
    ...
    def basic_cancel(self, consumer_tag):
        if not consumer_tag in self.callbacks:
            raise exceptions.UnknownConsumerTag(consumer_tag)
